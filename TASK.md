...
| #   | 事项                                       | 一句话说明                                                                                                      | 触发条件 / 建议归属                                                                                                                                                    |
| --- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | {待裁决项} 导出 HTML lang 属性硬编码 zh-CN | 导出产物 `<html lang="zh-CN">` 为 09 首版自定默认值；非中文文档导出时 lang 标注失真                             | 是否暴露设置项 / 跟随文档语言，归 10 设置迭代（12 D2 裁决不接：menuRouter 仅经 getExportMenuEntries/runExport 装配，不触碰 HTML 模板——2026-09-12 PR #18 核实维持登记） |
...
## 10 设置快捷键模块登记项（2026-09-08 计划登记；2026-09-12 PR #18 补验收口）

> 已闭环（2026-09-12 PR #18 合入 test 4251558）：原 #1 菜单侧补验——menuRouter 消费 `getMenuShortcutEntries` 装配菜单 label + 窗口域 keyBinding 按键执行通路（window-keybinding-shortcuts.ts），keybinding-restart.e2e.ts 实证 conf.user.json 配置 `"Always on Top": "Ctrl+Shift+P"` → 重启装载 → 按键置顶/还原端到端；原 #2 窗口域默认组合一致性——12 菜单装配逐项核对一致（catalog 为单一事实源，默认组合未改）；原 #3 defaultPrevented 守卫——按五服务（app/tabs/search/file-tree/devtools）全部补齐。

| #   | 事项                                                  | 一句话说明                                                                                                                                                                                    | 触发条件 / 建议归属                                      |
| --- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| 4   | {待裁决项} Ctrl+O / Ctrl+Shift+S 键盘通路未入 catalog | 12 PR #18 新增打开/另存为键盘通路已生效（菜单 label 已标注组合），但 shortcut-catalog（设置界面快捷键单一事实源）无对应条目——设置界面不可见亦不可自定义（同批新增 Ctrl+Shift+N/0/=/- 亦未入） | 10 域迭代扩 catalog 条目时拍板（PR #18 披露第 9 条移交） |
...
> 已闭环（2026-09-12 PR #18 P4 E2E 交付）：原 #4「源码模式 E2E 未跑（归 P4）」——window-shell.e2e.ts 已含 AC-M-6/7/8 源码往返用例（切入高亮/切回光标恢复/编辑切回同步），PR #18 内全量 E2E 两轮全绿，删除该条。

| #   | 事项                                   | 一句话说明                                                                                                                                                                                                                          | 触发条件 / 建议归属                                                                     |
| --- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| 1   | 源码模式 Tab/Shift+Tab 缩进均未绑定    | 调研报告 §2.1 提及的源码层缩进键位，12 spec AC 未列——首版未实现（I2 更正：defaultKeymap/historyKeymap 无任何 Tab 绑定，Tab 现会触发浏览器焦点移动；增强批次装配 indentWithTab 时须一并评估焦点逃逸）                                | 后续源码模式增强批次；需先入 12 spec 再实现，装配 indentWithTab 时同步处置 Tab 焦点逃逸 |
| 2   | 源码层未回写编辑随标签切换丢弃         | 源码模式下切换标签时，04 无「按标签写回编辑器」接口（门面已切新标签），源码层未回写编辑随刷新丢弃——console.warn + 层内浮层可见提示（I3 短期方案）；单标签内切出（Ctrl+/）回写不受影响；关窗路径已由 close-flush 回写兜底（6065a2f） | 04 提供 per-tab 写回接口（同 saveTab 缺口 C 性质）后闭环；届时可去掉丢弃降级路径        |
| 3   | 源码模式内容同步为切出时机（MVP 口径） | 仅在切出（Ctrl+/ 回所见即所得）时经 setContent 整体回写，未做逐键双向实时同步（spec §2 定案的 MVP 范围）；源码层挂载期间 WYSIWYG 侧的自动保存流不感知源码层编辑（关窗出口已由 close-flush 覆盖）                                    | 如需实时同步另立工作包（涉及防抖预算与 undo 语义重设计，不属缺陷）                      |
