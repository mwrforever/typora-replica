...
- **双向切换**：Ctrl+/（或菜单「视图 → 源码模式」）在所见即所得与 markdown 源码之间切换；切换双向恢复光标位置（落在原位置同一行/字符附近）。
- **源码编辑**：CodeMirror 6 承载，带 markdown 语法高亮、自动换行、等宽字体；亮/暗主题跟随系统色系。
- **内容同步**：源码模式下的编辑在切回所见即所得时整体回写（未编辑则不回写）；回写保留撤销栈（Ctrl+Z 一次即可撤销整次同步）。
- **保活语义**：所见即所得编辑器与源码层双实例常驻，切换零重建；标签切换时源码层自动跟随活跃标签刷新。

## 功能说明：Focus / 打字机模式（12 模块·W5）

- **专注模式**：F8（或菜单「视图 → 专注模式」，勾选项）开启后淡化光标所在块之外的内容，当前块及其容器链（引用/列表等）保持高亮；淡化强度走主题 CSS 的 `--blur-text-color` 变量，与 Typora 主题契约类名同名实现（自定义主题可直接复用）。
- **打字机模式**：F9（或菜单「视图 → 打字机模式」，勾选项）开启后输入时文档自动滚动，光标保持屏幕垂直居中；鼠标点击其他位置默认同样滚动居中。
- **点击居中偏好**：偏好设置 Appearance 分区的「打字机模式点击滚动居中」可关闭点击滚动（关闭后仅输入时滚动），即时生效。
- **模式叠加**：专注与打字机两模式独立开关、可同时生效；源码模式下专注模式同样生效（淡化光标所在行以外的内容）。
