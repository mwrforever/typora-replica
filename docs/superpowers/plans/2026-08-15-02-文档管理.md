...
- **全部文件 IO 走 Rust command**（方案 C，前端禁用 fs 插件静态 scope）；`read_file/write_file/list_dir/save_draft/list_drafts/recover_draft/watch_dir/get_cli_args` 八个命令签名锁定（spec §5，03/04/06/09/12 消费，勿改）。
...
| `src-tauri/src/io/commands.rs` [新建]                   | 八个 `#[tauri::command]` 薄层 + `parse_cli_args`（纯函数）                       |
| `src/services/file-io.ts` [新建]                        | invoke 封装（七命令）+ 中文错误规范化                                            |
...
- Produces（后续任务依赖，签名不得改动）:
  ...
  - `commands.rs` 声明八个命令函数签名（实现分布到后续任务）：
...
`src-tauri/src/io/commands.rs`（骨架，八个命令分步填充）：
...
- Consumes: 八个 Rust command（Task 1/5/6/7 已注册）
...
git commit -m "feat(02): 文件 IO 桥与偏好设置——八命令注册/七封装/错误规范化/store 持久化/默认开裁决"
```
