...
| #   | 功能            | 行为规范                                                                                                                                | 实现机制                                     |
| --- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| F27 | Focus 模式      | F8 淡化当前行/块以外内容；DOM 契约类名同名实现（on-focus-mode/md-focus/--blur-text-color）                                              | CSS 类切换                                   |
| F28 | Typewriter 模式 | F9 光标保持屏幕中部、文档滚动；默认鼠标点击也居中（偏好可关）                                                                           | 滚动同步逻辑                                 |
...
| S3  | 字数统计 | 状态栏显示（默认字数）；点击弹面板（行数/字数/字符数/阅读时间）**可切换计数单位**；选中统计（选中 N/总 N）；**中文一字一词**；代码块文本计入；字符数渲染级口径；拼写/词典图标首版占位；绑定活动标签页 | ProseMirror doc 文本统计         |
...
| Source Code Mode                 | Ctrl+/               | 源码模式（双向切换恢复光标、语法高亮；官方称谓 hybrid editing mode） |
| Focus Mode                       | F8                   | 专注模式（F27）                                                      |
| Typewriter Mode                  | F9                   | 打字机模式（F28）                                                    |
| Toggle Fullscreen                | F11                  | 全屏（进入全屏自动隐藏菜单栏；重启恢复窗口状态）                     |
| Actual Size / Zoom In / Zoom Out | Ctrl+Shift+0 / = / - | 界面缩放（范围自定 50%-200%，整窗含侧栏）                            |
