...
// 注入防御：slugOf 只做小写/空白折叠、保留引号与尖括号（typora-heading-id.ts），
// id 与 text 拼入 HTML 前统一经 escapeHtmlValue 转义，href 属性值无法提前闭合注入事件处理器。
import { escapeHtmlValue } from "./yaml-variables";
...
function buildFlat(headings: HeadingInfo[]): string {
  return headings
    .map((h) => `<li><a href="#${escapeHtmlValue(h.id)}">${escapeHtmlValue(h.text)}</a></li>`)
    .join("\n");
}
...
function buildLevel(headings: HeadingInfo[], level: number): { html: string; rest: HeadingInfo[] } {
  ...
  while (i < headings.length && headings[i]!.level >= level) {
    ...
    if (heading.level === level) {
      ...
      lines.push(
        `<li><a href="#${escapeHtmlValue(heading.id)}">${escapeHtmlValue(heading.text)}</a>\n${subHtml}</li>`,
      );
      ...
      lines.push(
        `<li><a href="#${escapeHtmlValue(heading.id)}">${escapeHtmlValue(heading.text)}</a></li>`,
      );
      i += 1;
    }
  }
