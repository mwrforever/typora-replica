...
it("计数文案三态：n/m、0 匹配 0/0、非法正则专属提示（AC-F24-2/6、AC-F25-5 呈现）", async () => {
...
it("三开关按钮切换 active 类与 store 字段；replace 模式下替换行暂不渲染（Task 6 增量）", async () => {
  const { store, wrapper } = await openPanel();
  // replace 行占位（Task 6 增量）：find 态与当前态均不渲染替换输入
