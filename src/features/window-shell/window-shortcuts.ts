...
// 12 W4：Ctrl+/ 源码模式双向切换（AC-M-6~8）；
// 12 W5：F8 专注模式 / F9 打字机模式（AC-M-10~12）
//
// 菜单装配引入的窗口级组合（此前无键盘通路，菜单 label 真实性要求注册）；
// Ctrl+Shift+N/0/=/- 与 Ctrl+/ 同步登记进 10 域 WINDOW_RESERVED_PM_KEYS
//（shortcut-keys.ts，防 keyBinding 把编辑器命令绑到同名组合造成双重执行；
// F11/F8/F9 无 Ctrl 修饰本就被 parseShortcutCombo 拒绝，无需入集）。与 02
// registerAppShortcuts（纯 Ctrl）同型分层：编辑器内快捷键归 01 keymap 注册表，
// 本层只处理窗口级组合；defaultPrevented 守卫随 10#3 收口先行。
// 键位口径：菜单标注组合串为美式键位约定（与 00 spec §11 表一致）；本层按
// event.code 物理键位判定，非美式布局键帽文案不同但物理位置等价触发。
// Ctrl+/ 例外按 event.key 判定："/" 无布局漂移问题且 Shift+Slash（"?"）需排除。
...
/**
 * 无修饰单键判定（F11/F8/F9 功能键族共用：任何 Ctrl/Alt/Meta/Shift 修饰组合
 * 均不归本服务，保留系统与编辑器组合语义）
 */
function isBareKey(event: KeyboardEvent, key: string): boolean {
  return event.key === key && !event.ctrlKey && !event.altKey && !event.metaKey && !event.shiftKey;
}
...
export function registerWindowShellShortcuts(handlers: WindowShellShortcutHandlers): () => void {
  const onKeydown = (event: KeyboardEvent): void => {
    ...
    // 功能键族（F11 全屏 / F8 专注 / F9 打字机）：单键无修饰
    if (isBareKey(event, "F11")) {
      // 拦截 WebView 默认行为（如原生全屏语义），统一走本命令函数
      event.preventDefault();
      handlers.onToggleFullscreen();
      return;
    }
    if (isBareKey(event, "F8")) {
      event.preventDefault();
      handlers.onToggleFocusMode();
      return;
    }
    if (isBareKey(event, "F9")) {
      event.preventDefault();
      handlers.onToggleTypewriterMode();
      return;
    }
    ...
    if (key === "o" && !event.shiftKey) {
      ...
      // 源码模式（12 W4，AC-M-6~8）：Ctrl+/ 双向切换；Shift+Slash（"?"）不归本服务
      event.preventDefault();
