...
// 07 图片上传、09 导出、10 设置快捷键、11 字数统计、12 标题栏脏状态消费。
//
...
class EditorManager {
  ...
  destroy(): void {
    ...
    if (this.crepe) {
      ...
      // 抛错即整链 reject——无 catch 会让 pendingDestroy 恒 reject，此后每次 create
      // 在 await 处同步失败且无恢复路径（单例永久毒化）。catch 兜底记录后恒 resolve
      this.pendingDestroy = Promise.resolve(this.crepe.destroy()).catch((error) => {
        console.error("[MarkWell] 编辑器销毁失败（插件清理异常）:", error);
      });
